        ...
        <div class="content-box-header">
            ...
            <div class="content-box-btn">
                ...
                @can('edit-password-user')
                    <a href="{{ route('users.edit_password', ['user' => $user->id]) }}" class="btn-warning align-icon-btn">
                        <!-- Ícone pencil-square (Heroicons) -->
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="size-5">
                            <path stroke-linecap="round" stroke-linejoin="round"
                ...
                @can('destroy-user')
                    <form id="delete-form-{{ $user->id }}" action="{{ route('users.destroy', ['user' => $user->id]) }}" method="POST">
                        ...
                        <button type="button" onclick="confirmDelete({{ $user->id }})" class="btn-danger flex items-center space-x-1">
            ...
            <div class="mb-1">
                ...
                <span class="detail-content">
                    @can('index-user-status')
                        <a href="{{ route('user_statuses.show', ['userStatus' => $user->userStatus->id]) }}">{{ $user->userStatus->status }}</a>
                    @elsecan()
